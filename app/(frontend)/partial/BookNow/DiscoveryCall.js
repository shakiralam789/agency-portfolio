...
const DiscoveryCall = forwardRef(({ closeAnim }, ref) => {
  ...
  useEffect(() => {
    ...
    // Initialize modal scroll
    if (contentRef.current) {
      modalLenisRef.current = new Lenis({
        wrapper: document.getElementById('book-now-modal-content'),
  ...
  return (
    ...
    >
      ...
      <div className="relative active max-w-[800px] 2xl:max-w-[1000px] w-full opacity-0 pointer-events-none [&.active]:opacity-100 [&.active]:pointer-events-auto bg-white z-30 h-full">
        <div className="h-full grid grid-cols-12 gap-2 2xl:gap-4">
          <div className="col-span-12 md:col-span-5 border-r h-full overflow-y-auto">
            <div className="p-4 2xl:p-5 pr-0 mr-4 md:mr-0">
          ...
          <div 
            id="book-now-modal-content" 
            className="col-span-12 md:col-span-7 h-full"
          >
            <div 
              ref={contentRef}
              className="h-full"
